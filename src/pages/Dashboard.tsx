...
const Dashboard: React.FC = () => {
    ...
    const [lastStudiedSet, setLastStudiedSet] = useState<{
        setId: string;
        setTitle: string;
    ...
    // Fetch last studied set
    useEffect(() => {
        const fetchLastStudied = async () => {
            if (!user) return;
            try {
                const lastSet = await progressApi.getLastStudiedSet(user.id);
                setLastStudiedSet(lastSet);
            } catch (error) {
                console.error('Error fetching last studied set:', error);
            }
        };
        fetchLastStudied();
    }, [user]);
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 @container">
                        ...
                        <div className="flex flex-col items-stretch justify-start rounded-xl shadow-[0_0_12px_rgba(0,0,0,0.05)] bg-white dark:bg-slate-900 overflow-hidden ring-1 ring-slate-200 dark:ring-slate-800">
                            <div
                                ...
                                style={{
                                    backgroundImage: `url("${lastStudiedSet?.imageUrl || 'https://lh3.googleusercontent.com/aida-public/AB6AXuBEhsL1IL7W4V0EOOtwHCycGVy97O8l_9cOw4bn-q8ZSPvmIAXwqgveWC_sOl0kiK-DWsiM_XZdPYsX9xd7z5zp85KNET3ySjmbJ23sXKSOMe-1LFqj-zXjX4oK3OrnK523fW2djHaKjTSihcRlcR1_Ur7VsByosAkiGyy_vspeStWegRp1nRCY4_UHkHS4hXnvsDurzpLfHCt8fxWmF8AkFuryAtybUoT8S0lnNysVGMb9WV0exJXgIN8QDPHu2Q_UsYnqtaUSZMt1'}")`
                                }}
                            ></div>
                            <div className="flex w-full min-w-72 grow flex-col items-stretch justify-center gap-4 p-6">
                                <p className="text-[#0d171b] dark:text-slate-50 text-lg font-bold leading-tight tracking-[-0.015em]">
                                    {lastStudiedSet ? 'Continue where you left off' : 'Start your learning journey'}
                                </p>
                                <div className="flex flex-col gap-1">
                                    <p className="text-slate-500 dark:text-slate-400 text-base font-normal leading-normal">
                                        {lastStudiedSet ? lastStudiedSet.setTitle : 'Explore vocabulary sets and start learning today!'}
                                    </p>
                                    {lastStudiedSet && (
                                        <p className="text-slate-500 dark:text-slate-400 text-base font-normal leading-normal">
                                            You are {lastStudiedSet.progress}% through this lesson.
                                        </p>
                                    )}
                                </div>
                                {/* Progress Bar */}
                                {lastStudiedSet ? (
                                    <div className="flex flex-col gap-2">
                                        <div className="rounded-full bg-slate-200 dark:bg-slate-700 h-2 w-full">
                                            <div className="h-2 rounded-full bg-primary" style={{ width: `${lastStudiedSet.progress}%` }}></div>
                                        </div>
                                    </div>
                                ) : null}
                                <div className="flex items-end justify-between gap-3 pt-2">
                                    <Link
                                        to={lastStudiedSet ? `/sets/${lastStudiedSet.setId}` : '/explore'}
                                        className="flex w-full sm:w-auto min-w-[84px] max-w-[480px] cursor-pointer items-center justify-center overflow-hidden rounded-lg h-10 px-4 bg-primary text-slate-50 text-sm font-medium leading-normal hover:opacity-90 transition-opacity"
                                    >
                                        <span className="truncate">{lastStudiedSet ? 'Resume Learning' : 'Explore Sets'}</span>
                                    </Link>
                                </div>
                            </div>
                        </div>
